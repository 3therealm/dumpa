...
def run(cmd: list[str],
    ...
    `quiet=True` is for callers that tolerate a non-zero exit (opportunistic probes
    like `info`'s signer/badging reads): the failure is logged at debug, not error,
    but the exception is still raised so the caller can decide what to do.
    """
    log_fail = logger.debug if quiet else logger.error
    ...
    with tempfile.TemporaryFile() as stdout_file, tempfile.TemporaryFile() as stderr_file:
        try:
            ...
            log_fail("command timed out after %ss: %s", resolved_timeout, format_command(cmd))
            if stderr_tail:
                log_fail("stderr tail:\n%s", stderr_tail)
            if stdout_tail:
        ...
        if proc.returncode != 0:
            stderr_tail = _read_output_tail(stderr_file)
            stdout_tail = _read_output_tail(stdout_file, max_lines=20)
            log_fail("command failed (rc=%s): %s", proc.returncode, format_command(cmd))
            if stderr_tail:
                log_fail("stderr tail:\n%s", stderr_tail)
            if stdout_tail:
                log_fail("stdout tail:\n%s", stdout_tail)
